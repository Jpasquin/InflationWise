...
<div ref="reportContainer">
  <div class="grid grid-cols-1 gap-4 pb-[74px]" ref="debtsContainer">
    ...
    >
      ...
      <ol v-if="section.isList" class="text-2xl text-left my-2">
        <li v-for="(item, itemIndex) in section.description" :key="`item-${itemIndex}`">
          {{ item }}
        </li>
      </ol>
    ...
    strategiesComputed() {
      ...
      while ((match = pattern.exec(this.strategies))) {
        ...
        if (description.match(/^\d+\./)) {
          listItems = description.split(/\d+\./).map(s => s.trim()).filter(s => s);
        }
